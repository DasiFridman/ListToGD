...
if (empty($organized_data)) {
    ...
    $all_urls = get_option('ltgdai_saved_urls', array());
    
    if (empty($all_urls) || empty(array_filter($all_urls))) {
        echo '<div class="ltgdai-panel">';
        echo '<div class="ltgdai-panel-header">';
        echo '<h2>אין נתונים למיפוי</h2>';
        echo '</div>';
        echo '<div class="ltgdai-panel-content">';
        echo '<p style="font-size: 16px; margin-bottom: 20px;">נראה שטרם הוספת נתונים למערכת או שכל הנתונים כבר הועברו בהצלחה!</p>';
        echo '<div style="text-align: center;">';
        echo '<a href="' . admin_url('admin.php?page=ltgdai-import') . '" class="ltgdai-button ltgdai-mapping" style="font-size: 16px; padding: 12px 30px;">התחל עם נתונים חדשים</a>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
    } else {
        echo '<div class="ltgdai-panel">';
        echo '<div class="ltgdai-panel-header">';
        echo '<h2>כל המיפויים הושלמו!</h2>';
        echo '</div>';
        echo '<div class="ltgdai-panel-content">';
        echo '<p style="font-size: 16px; margin-bottom: 20px;">מעולה! כל הנתונים שהוספת כבר עברו את תהליך המיפוי וההזנה בהצלחה.</p>';
        echo '<div style="background: #f0f7ff; border: 1px solid #0073aa; padding: 15px; border-radius: 6px; margin-bottom: 20px;">';
        echo '<h3 style="margin: 0 0 10px; color: #0073aa;">מה אפשר לעשות עכשיו?</h3>';
        echo '<ul style="margin: 0; padding-right: 20px;">';
        echo '<li>להוסיף נתונים חדשים ולהתחיל תהליך חדש</li>';
        echo '<li>לבדוק את הפוסטים שנוצרו באתר</li>';
        echo '<li>לנהל את הנתונים שכבר הועברו</li>';
        echo '</ul>';
        echo '</div>';
        echo '<div style="text-align: center;">';
        echo '<a href="' . admin_url('admin.php?page=ltgdai-import') . '" class="ltgdai-button ltgdai-mapping" style="font-size: 16px; padding: 12px 30px;">הוסף נתונים חדשים</a>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
    }
    ...
    // חילוץ נתונים אוטומטי משופר
    if ($needs_extraction && !empty($missing_urls)) {
        require_once LTGDAI_PLUGIN_DIR . 'classes/ContentExtractor.php';
        $extractor = new ContentExtractor();
        
        $extracted_count = 0;
        $total_urls = count($missing_urls);
        
        error_log("מתחיל חילוץ אוטומטי עבור $total_urls URLs בטאב $current_tab");
        
        foreach ($missing_urls as $url) {
            try {
                error_log("מחלץ אוטומטית מ-URL: $url");
                            ...
                            foreach ($first_table['rows'] as $row) {
                                foreach ($row as $cell) {
                                    if (filter_var($cell, FILTER_VALIDATE_URL) && 
                                        (preg_match('/\.(pdf|doc|docx|xls|xlsx)(\?|$)/i', $cell) ||
                                         preg_match('/\.ashx.*FileID/i', $cell))) {
                                        $file_links_found++;
                                    }
                                }
                            }
                            if ($file_links_found > 0) {
                                error_log("נמצאו {$file_links_found} קישורי קבצים ב-URL: {$url}");
                            }
                        }
                    }
                } else {
                    error_log("שגיאה בחילוץ אוטומטי מ-URL: {$url} - " . $extracted_data['message']);
                }
                
            } catch (Exception $e) {
                error_log("Exception בחילוץ אוטומטי עבור URL: {$url} - " . $e->getMessage());
            }
        }
        
        if ($extracted_count > 0) {
            error_log("חילוץ אוטומטי הושלם - חולצו נתונים מ-{$extracted_count} מתוך {$total_urls} קישורים");
        }
    }
...
function ltgdai_process_mapping_form_submission() {
    if (isset($_POST['ltgdai_extract_data']) || isset($_POST['ltgdai_ready_for_import']) || isset($_POST['ltgdai_save_mapping']) || isset($_POST['ltgdai_reset_mappings'])) {
        ...
        error_log('עיבוד שליחת טופס מיפוי');
        error_log('טאב נוכחי: ' . $current_tab);
        error_log('נתוני POST: ' . print_r($_POST, true));
        ...
        if (isset($_POST['ltgdai_ready_for_import']) || isset($_POST['ltgdai_save_mapping'])) {
            ...
            $url_mappings = isset($_POST['ltgdai_url_mapping']) ? $_POST['ltgdai_url_mapping'] : array();
            
            error_log('מיפויי שדות מהטופס: ' . print_r($field_mappings, true));
            error_log('שמות שדות מותאמים: ' . print_r($custom_field_names, true));
            error_log('מיפויי URL: ' . print_r($url_mappings, true));
            ...
            foreach ($field_mappings as $url_key => $mappings) {
                ...
                foreach ($mappings as $source_field => $gd_field) {
                    if (!empty($gd_field)) {
                        ...
                        error_log("מיפוי: שדה GD '$gd_field' <- שדה מקור '$source_field'");
                    }
                }
            }
            ...
            foreach ($custom_field_names as $url_key => $custom_names) {
                ...
                foreach ($custom_names as $index => $custom_name) {
                    if (!empty(trim($custom_name))) {
                        $custom_field_key = 'custom_field_' . ($index + 1);
                        ...
                        error_log("שדה מותאם: '$custom_name' -> שדה GD '$gd_field'");
                    }
                }
            }
            ...
            error_log('נתוני מיפוי סופיים לשמירה: ' . print_r($mapping_data, true));
            
            $save_result = $mapping_handler->save_field_mappings($current_tab, $mapping_data);
            ...
            if ($save_result) {
                ...
                if (!in_array($current_tab, $completed_tabs)) {
                    ...
                    update_option('ltgdai_completed_tabs', $completed_tabs);
                    error_log('טאב סומן כהושלם: ' . $current_tab);
                }
                ...
                echo '<div class="notice notice-error is-dismissible"><p>שגיאה בשמירת המיפוי. נסה שוב.</p></div>';
            }
        }
    }
}
